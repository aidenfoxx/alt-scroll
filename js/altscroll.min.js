...
AltScroll.prototype.scrollTo=function(a,b,c,e){var d=this.contentRect.width-this.containerRect.width+this.scrollbarSize,h=this.contentRect.height-this.containerRect.height+this.scrollbarSize;if(0>a||a>d){var f=a-this.container.scrollLeft;a=0>a?0:d;f=Math.abs(a-this.container.scrollLeft)/Math.abs(f)}if(0>b||b>h){d=b-this.container.scrollTop;b=0>b?0:h;var g=Math.abs(b-this.container.scrollTop)/Math.abs(d)}!f&&g?c*=g:f&&!g?c*=f:f&&g&&(c*=f<g?f:g);window.cancelAnimationFrame(this.scrollFrame);this.scrollFrame=
window.requestAnimationFrame(function(){this.scrollToFrame(Date.now(),this.container.scrollLeft,a,this.container.scrollTop,b,c,e)}.bind(this))};
...
AltScroll.prototype.snapToNearest=function(a,b){return this.snapTo(this.calcNearestChild(),a,b)};AltScroll.prototype.scroll=function(a){this.options.snap&&(clearTimeout(this.scrollTimeout),this.scrollTimeout=setTimeout(function(){this.container.removeEventListener("scroll",this.scrollEvent);this.scrollEvent=null;this.snapToNearest()}.bind(this),500))};
AltScroll.prototype.dragStart=function(a){a.preventDefault();this.dragEvent||a.sourceCapabilities&&a.sourceCapabilities.firesTouchEvents||(this.dragBegin=Date.now(),this.dragInitMouseVec=this.calcTouchCoords(a),this.dragInitVec={x:this.container.scrollLeft,y:this.container.scrollTop},this.dragEvent=this.drag.bind(this),this.container.addEventListener("mousemove",this.dragEvent))};
AltScroll.prototype.drag=function(a){a.preventDefault();a=this.calcTouchCoords(a);this.scrollTo(this.dragInitVec.x+(this.dragInitMouseVec.x-a.x),this.dragInitVec.y+(this.dragInitMouseVec.y-a.y),0)};
