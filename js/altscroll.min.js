...
AltScroll.prototype.bindEvents=function(){"ontouchstart"in window?(this.container.addEventListener("touchstart",this.touchStart.bind(this)),document.addEventListener("touchend",this.touchEnd.bind(this)),document.addEventListener("touchcancel",this.touchEnd.bind(this))):0<navigator.maxTouchPoints?(this.container.addEventListener("pointerdown",this.touchStart.bind(this)),document.addEventListener("pointerup",this.touchEnd.bind(this)),document.addEventListener("pointercancel",this.touchEnd.bind(this))):
0<navigator.msMaxTouchPoints&&(this.container.addEventListener("MSPointerDown",this.touchStart.bind(this)),document.addEventListener("MSPointerUp",this.touchEnd.bind(this)),document.addEventListener("MSPointerCalcel",this.touchEnd.bind(this)));this.container.addEventListener("mousedown",this.dragStart.bind(this));document.addEventListener("mouseup",this.dragEnd.bind(this));this.options.snap?(this.container.addEventListener("mousewheel",this.snapDelay.bind(this)),this.container.addEventListener("DOMMouseScroll",
this.snapDelay.bind(this))):(this.container.addEventListener("mousewheel",this.scrollStop.bind(this)),this.container.addEventListener("DOMMouseScroll",this.scrollStop.bind(this)));this.options.dynamicResize&&window.addEventListener("resize",this.resize.bind(this))};AltScroll.prototype.resize=function(){clearTimeout(this.resizeTimeout);this.resizeTimeout=setTimeout(function(){this.resizeCallback()}.bind(this),500)};
AltScroll.prototype.resizeCallback=function(){this.hideScrollbars();this.containerRect=this.container.getBoundingClientRect();this.contentRect=this.content.getBoundingClientRect();this.childRect=this.getChildRect()};
...
AltScroll.prototype.dragEnd=function(a){if(this.dragEvent){var b=this.calcTouchCoords(a);a=this.dragInitMouseVec.x-b.x;b=this.dragInitMouseVec.y-b.y;if(this.options.momentum&&(a||b)){var c=Date.now()-this.dragBegin;a/=c;b/=c;this.scrollTo(this.container.scrollLeft+Math.abs(a)*a/(2*this.options.momentumFalloff),this.container.scrollTop+Math.abs(b)*b/(2*this.options.momentumFalloff),Math.abs(Math.abs(a)>Math.abs(b)?a:b)/this.options.momentumFalloff,"object"===typeof EasingFunctions?EasingFunctions.easeOutCubic:
null,this.options.snap?this.snapToNearest:null)}else this.options.snap&&this.snapToNearest();this.container.removeEventListener("mousemove",this.dragEvent);this.dragEvent=null}};