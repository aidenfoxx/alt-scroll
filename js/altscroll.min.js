...
AltScroll.prototype.bindEvents=function(){this.dragStartEvent=this.dragStart.bind(this);this.container.addEventListener("touchstart",this.touchStart.bind(this));window.addEventListener("touchend",this.touchEnd.bind(this));window.addEventListener("touchcancel",this.touchEnd.bind(this));this.container.addEventListener("mousedown",this.dragStartEvent);window.addEventListener("mouseup",this.dragEnd.bind(this));window.addEventListener("resize",this.resize.bind(this));this.options.snap?(this.container.addEventListener("mousewheel",
this.snapDelay.bind(this)),this.container.addEventListener("DOMMouseScroll",this.snapDelay.bind(this))):(this.container.addEventListener("mousewheel",this.scrollStop.bind(this)),this.container.addEventListener("DOMMouseScroll",this.scrollStop.bind(this)))};AltScroll.prototype.resize=function(){clearTimeout(this.resizeTimeout);this.resizeTimeout=setTimeout(function(){this.resizeCallback()}.bind(this),500)};
...
AltScroll.prototype.touchStart=function(){this.container.removeEventListener("mousedown",this.dragStartEvent);this.dragStartEvent=null;this.scrollStop()};AltScroll.prototype.touchEnd=function(){this.dragStartEvent||(setTimeout(function(){this.dragStartEvent=this.dragStart.bind(this);this.container.addEventListener("mousedown",this.dragStartEvent)}.bind(this),50),this.options.snap&&this.snapDelay())};
AltScroll.prototype.dragStart=function(a){a.preventDefault();this.scrollStop();this.dragBegin=Date.now();this.dragInitMouseVec=this.calcTouchCoords(a);this.dragInitVec={x:this.container.scrollLeft,y:this.container.scrollTop};this.dragEvent=this.drag.bind(this);this.container.addEventListener("mousemove",this.dragEvent)};AltScroll.prototype.drag=function(a){a=this.calcTouchCoords(a);this.scrollTo(this.dragInitVec.x+(this.dragInitMouseVec.x-a.x),this.dragInitVec.y+(this.dragInitMouseVec.y-a.y),0)};
